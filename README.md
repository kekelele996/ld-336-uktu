...
1. **开始维修互斥**：开始维修前，同一设备存在其他"待处理"或"处理中"维修工单时，本次开始**整次拒绝**（返回 409 与阻塞原因，含阻塞工单号与状态）。重复报修产生的多张待处理工单需先取消多余工单，再开始原工单。
2. **维修期间冻结调拨/报废**：设备处于"维修中"时，发起调拨/报废申请、审批既有调拨/报废申请一并拒绝，校验与写入在同一事务内基于设备行锁完成，**原工单、设备与申请保持原样**（申请不会落库）。
3. **完成维修按计量结果恢复**：完成维修时读取设备最新计量结果——最新结果为**不合格**则设备**保持禁用**并提示"未通过计量"；只有计量**合格**或设备**从未纳入计量**时才恢复"使用中"。
4. **幂等与并发**：重复开始/并发提交只能生效一次（`SELECT ... FOR UPDATE` 设备行锁串行化，先提交者生效，后者收到确定性 409）；前端页面直接展示阻塞原因，并在每次操作后刷新，刷新后状态与后端一致。
