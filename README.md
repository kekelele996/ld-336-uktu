...
3. **维护与保养管理**：日检/周检/月检/年检保养计划自动生成与到期提醒，工程师执行并记录保养内容、更换配件、工时与费用；故障扫码快速报修与维修过程记录。同一设备存在待处理/处理中的维修工单时整次拒绝新报修，重复开始或并发提交只生效一次。
4. **计量与质控管理**：计量台账（器具编号、周期、上下次计量日期），到期预警清单，计量结果登记（不合格自动标记设备禁用）。维修完成时按最新计量结果恢复设备：计量合格或从未纳入计量才恢复"使用中"，最新计量不合格则保持禁用并提示未通过计量。
5. **设备调拨与报废**：科室间调拨申请审批后自动更新设备科室与责任人；报废审批通过后设备状态变更为"已报废"并归档。设备维修期间调拨/报废申请（发起与审批）一并拒绝，原维修工单、设备与申请保持原样。
